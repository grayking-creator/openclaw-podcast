...
9. **AgentStack Daily/OpenClaw Daily shorts are approved and active.** Toby re-approved shorts for this podcast on 2026-06-06. The `youtube_shorts_pipeline.py --mode cron` runs daily at 09:30 ET via cron and stages 2 EN short candidates per episode. Failures are reported to Discord `#build-log` (channel `1485243812442804327`). Do NOT re-disable the pipeline or add `SystemExit` guards without an explicit instruction from Toby. Full episode uploads and shorts staging run in separate processes to avoid resource contention.
