...
[NOVA]: Bug um: um usuário tinha MiniMax configurado como seu modelo de raciocínio. A API upstream estava retornando api_error. O OpenClaw olhou para isso, decidiu que devia ser transitório, tentou de novo em silêncio, nunca expôs a falha, nunca acionou o fallback corretamente, e o usuário recebeu de volta um resultado degradado sem ter ideia de que a chamada original tinha falhado.
...
[NOVA]: Exatamente. E o bug dois era uma classe diferente de dor, mas igualmente real. O usuário cola um token novo de OpenAI Codex, vê a confirmação, tudo parece bem-sucedido, reinicia o gateway, e o token volta para a credencial expirada.

[ALLOY]: Esse faz você se sentir maluco. Porque, da perspectiva do usuário, ele fez a coisa certa. Colou o token novo. O app disse: sim, salvo. Aí, depois do restart, não. Token antigo e ruim de novo. ... Por baixo dos panos, o estado de auth em memória do gateway estava sobrescrevendo o valor recém-salvo em disco no restart.

[NOVA]: Ambos foram corrigidos em v2026.3.23. Mas, para entender por que aconteceram, você precisa entender o que v2026.3.22 mudou. Porque a .22 é a grande. A .22 é o OpenClaw fazendo algo que provavelmente deveria ter feito há um ano.

[ALLOY]: O expurgo do legado.

[NOVA]: O expurgo do legado. Os nomes antigos, as camadas de compatibilidade, os caminhos de transição esquisitos, as muletas de browser relay, a geleia do plugin SDK — muita coisa foi arrancada. ... E acho que a forma certa de enquadrar essas duas releases juntas é esta: a .22 remove a pele morta, e a .23 garante que a pele nova não rache.
...
[NOVA]: Vamos começar pela mudança mais carregada emocionalmente, porque as pessoas se apegam de um jeito estranhíssimo a nomes, mesmo quando esses nomes já deveriam ter sido aposentados para sempre. Os nomes de ambiente CLAWDBOT_* e MOLTBOT_* sumiram. Não deprecated. Não tolerados com warning. Sumiram.

[ALLOY]: E eu quero desacelerar isso, porque se você roda OpenClaw em um laptop e em nenhum outro lugar, talvez ouça isso e pense: ok, é só renomear algumas variáveis, beleza. Esse não é o modo real de falha. O modo real de falha é você ter um arquivo .env antigo no Docker Compose, ou uma unit systemd toda empoeirada num VPS, ou alguma coisa num shell profile que você não olha há oito meses. Você faz upgrade, o OpenClaw sobe, e esses valores são ignorados em silêncio.

[NOVA]: Sem erro. Sem banner de migração.

[ALLOY]: Nada de “ei, eu vi CLAWDBOT_TOKEN e isso está obsoleto”. Só config faltando. De repente o auth não bate, o caminho do state não está onde você achava, talvez um plugin não carregue, talvez um token pareça ausente. ... Esse é o tipo de quebra que te pega às duas da manhã depois de um upgrade numa máquina em que você não mexia há um tempo.

[NOVA]: Dê grep nos seus arquivos de env. Em todo ambiente em que você roda OpenClaw. Todo host. Todo arquivo de Compose. Toda unit de startup. Todo bootstrap de shell. Se você tiver CLAWDBOT_* ou MOLTBOT_*, trate isso como quebrado até corrigir.

[ALLOY]: Mesma história com o diretório de state antigo ~/.moltbot. Esse caminho já não faz parte do futuro. Se o seu state ainda mora lá, o OpenClaw não vai inferir isso magicamente para você depois do upgrade. Mova para ~/.openclaw ou defina OPENCLAW_STATE_DIR explicitamente e encerre o assunto.
...
[NOVA]: Justo. ... A segunda grande breaking change na .22 é que ClawHub agora é first-class, e o significado prático disso é maior do que a formulação de marketing faz parecer. openclaw plugins install name agora verifica o ClawHub primeiro, e só cai para npm se o ClawHub não tiver o pacote.
...
[NOVA]: Também existem comandos nativos agora: openclaw skills search, openclaw skills install, openclaw skills update. ... E, para mim, isto é o OpenClaw finalmente alinhando o produto ao ecossistema que ele pretendia ter. O ClawHub sempre deveria ser a casa das skills. Esta release torna isso real em vez de aspiracional.
...
[NOVA]: Terceiro: o overhaul do Plugin SDK. Isso não é um ajustezinho nas bordas. openclaw/extension-api sumiu. Não existe shim de compatibilidade. A nova superfície é openclaw/plugin-sdk/*, com subpaths mais estreitos e limites muito mais claros.
...
[ALLOY]: E a descoberta de mensagens também mudou. describeMessageTool() agora é obrigatório. O fluxo antigo de listActions, getCapabilities, getToolSchema foi removido. ... Isso não é renomear. Isso é mudança de contrato.

[NOVA]: O novo SDK é realmente mais limpo. De verdade. Os imports são mais estreitos. A intenção é mais clara. O modelo de runtime é mais coerente. Mas você realmente precisa migrar. E se você é desenvolvedor de plugin, vá ler docs.openclaw.ai/plugins/sdk-migration. Não improvise de memória.

[ALLOY]: Eu quero sublinhar isso. Esta não é a semana para fazer a migração no improviso, na base do feeling.

[NOVA]: Quarto bloco: hardening de segurança. Algumas dessas coisas são invisíveis até o momento em que te salvam, o que significa que elas não vão receber tanto tempo de fala, mas importam. O exec sandbox agora bloqueia MAVEN_OPTS, SBT_OPTS, GRADLE_OPTS, ANT_OPTS, além de GLIBC_TUNABLES e DOTNET_ADDITIONAL_DEPS. ... Isso é basicamente uma varredura contra superfícies de injeção de runtime que as pessoas esquecem que existem.
...
[NOVA]: Também existe uma mudança sutil, mas inteligente, na allowlist: time agora é transparente na avaliação da allowlist. Então time ./approved-script se vincula ao script interno, não ao wrapper time.

[ALLOY]: Essa é muito prática. As pessoas envolvem comandos em time o tempo todo durante debugging. Antes, wrappers podiam criar edge cases estranhos de policy. Agora ele avalia a coisa que você realmente queria rodar.

[NOVA]: E o hardening de voice webhook também ficou mais rígido: rejeitar assinaturas ausentes do provider antes de ler o body, com limite pré-auth de 64KB e 5s. Isso é simplesmente boa higiene de perímetro. Não gaste recursos fazendo parse de lixo não autenticado.
...
[NOVA]: Precisamos dedicar tempo de verdade ao tooling de navegador, porque, para muita gente, esta é a maior mudança operacional individual de toda a dupla de releases. O relay legado da extensão do Chrome acabou. driver: "extension", assets bundled da extensão, browser.relayBindHost — tudo removido.

[ALLOY]: E, se você não estava por perto naquela era, aqui vai o que a extensão realmente era. O OpenClaw costumava distribuir uma extensão do Chrome que agia como relay para o CDP, o Chrome DevTools Protocol. Você instalava a extensão manualmente, concedia permissões ao navegador, e ela servia como ponte entre o OpenClaw e o browser.
...
[NOVA]: O modelo substituto é simplesmente melhor. O OpenClaw agora se conecta diretamente a uma instância do Chrome em execução ou a um profile de usuário usando mecanismos padrão de CDP. Sem precisar de extensão. Sem relay customizado no meio. ... Arquitetonicamente, isto é mais limpo. Menos camadas sob medida. Menos segredos escondidos no tooling.

[ALLOY]: Mas — e este é o alerta prático importante — se você fizer upgrade sem rodar openclaw doctor --fix antes, ou pelo menos logo depois, sua automação de navegador pode quebrar completamente. E nem sempre vai quebrar de um jeito óbvio. Você não necessariamente vai receber uma mensagem amigável dizendo “extension relay removido”. O mais provável é que simplesmente falhe ao conectar, ou entre em loop esquisito de consentimento, ou seu caminho de attach pareça meio vivo e depois morra.

[NOVA]: openclaw doctor --fix lê sua config atual e migra setups locais de browser no host para o modo moderno correto: existing-session ou user. Isso não é uma recomendação cosmética. Faz parte da migração.

[ALLOY]: E vale esclarecer os três modos agora. existing-session significa anexar a um Chrome já em execução. user significa iniciar com um profile de usuário. CDP puro para setups em Docker, headless, sandbox, remotos — isso permanece basicamente inalterado.
...
[NOVA]: Além disso, se você roda automação local de browser no host, deveria tratar doctor --fix como parte da migração do navegador, não como uma tarefa geral de manutenção. Ele está fazendo trabalho direcionado.

[ALLOY]: Sim. Não é housekeeping opcional. É passo de migração.

[NOVA]: E há uma lição maior nessa mudança de navegador também. Automação de browser é um daqueles domínios em que as pessoas toleram complexidade absurda porque o retorno é muito alto. Vão instalar uma extensão, fixar versão, abençoar profile, carregar flags de launch esquisitas, qualquer coisa, desde que o navegador obedeça. ... Mas todo workaround escondido vira dívida técnica com interface de usuário.
...
[NOVA]: Próximo assunto: geração de imagem. Este é menos dramático do que tooling de navegador, mas te conta muito sobre para onde o OpenClaw está indo. A skill bundled nano-banana-pro foi removida. Sumiu. Sem shim.

[ALLOY]: O que significa que, se você tinha workflows, prompts ou docs internos que chamam nano-banana-pro, encontre isso e substitua. É uma quebra dura. Não presuma que existe um alias esperando por você.

[NOVA]: A plataforma está padronizando em torno da ferramenta core image_generate. E, filosoficamente, eu acho isso exatamente certo. Uma ferramenta, backend configurável, superfície de invocação consistente. Melhor do que carregar um wrapper bundled de skill para sempre só porque as pessoas se acostumaram com o nome.

[ALLOY]: Desde que você defina a config key. Esta é a parte que as pessoas pulam. Você precisa de agents.defaults.imageGenerationModel.primary: "google/gemini-3-pro-image-preview". Se você não definir isso, o comportamento fica indefinido. E indefinido em terra de config nunca significa empolgante. Significa confuso.
...
[NOVA]: Crítica justa. ... Também há melhorias de marketplace embrulhadas dentro dessa mudança. Instalações via marketplace agora são first-class, incluindo sintaxe plugin@marketplace e suporte ao registry de marketplace do Claude.
...
[NOVA]: E comandos de chat /plugins e /plugin restritos ao owner continuam exatamente essa mesma linha: dar à plataforma uma história coerente única para descobrir, instalar e gerenciar extensões.
...
[NOVA]: Para mim, a história da geração de imagem é esta: o OpenClaw está saindo de personalidade bundled e indo para capacidade configurada. ... Isso é maturidade.
...
[NOVA]: Também muda como as equipes deveriam pensar sobre portabilidade. Se o seu workflow diz image_generate e seu backend é configurado separadamente, você pode migrar providers sem reescrever a lógica do workflow em si.
...
[NOVA]: O problema original era classificação. Respostas genéricas de api_error do MiniMax estavam sendo tratadas como transitórias por padrão. Então o OpenClaw tentava de novo em silêncio, suprimia a falha real e nunca acionava o fallback corretamente quando o problema subjacente era algo como billing, auth ou contexto malformado.
...
[ALLOY]: Então você colava o token, via verde, reiniciava, expirado. Toda vez. ... É por isso que esse bug parecia tão pessoal. Ele atacava a confiança no ato básico de salvar credenciais.
...
[NOVA]: Exemplo concreto: você agenda um job para 8 AM. Chega o DST. Antes da .23, esse “8 AM” podia virar 7 AM ou 9 AM, dependendo de como o scheduler interpretava a fronteira. ... Isso não é só um desencontro cosmético. Para uma rotina diária, é uma promessa quebrada.

[ALLOY]: A correção é que --at --tz agora honra o horário de relógio local ao atravessar fronteiras de DST. E o OpenClaw também rejeita --tz com --every, o que é bom porque semântica de intervalo recorrente e semântica de horário local com timezone não são a mesma coisa.
...
[NOVA]: Quarto: a correção de 422 do Mistral. Configs persistidas antigas do Mistral vinham carregando limites de output do tamanho do contexto que o Mistral rejeita de imediato. Resultado: erros 422 que parecem misteriosos se você não conhece a linhagem da config.

[ALLOY]: E, mais uma vez, openclaw doctor --fix está fazendo trabalho real aqui. Agora ele detecta e corrige essas configs stale do Mistral.

[NOVA]: Mais um motivo para rodar doctor --fix. As pessoas às vezes ouvem esse comando como uma espécie de consulta genérica, tipo talvez ele arrume umas coisas óbvias. Não. Nesta sequência de releases, ele está codificando conhecimento de migração.

[ALLOY]: Quinto: ClawHub no macOS. Duas questões aqui. Credenciais salvas não estavam respeitando corretamente o caminho do macOS Application Support, e o comportamento de browse-all estava batendo em rate limits 429 sem autenticação.

[NOVA]: O que significa que a UI podia te induzir a pensar que o ClawHub estava vazio ou quebrado de maneiras vagas.

[ALLOY]: A navegação de skills estava caindo silenciosamente para modo não autenticado no macOS. Você via listas vazias e assumia que não existiam skills, ou achava que sua instalação estava quebrada, quando, na verdade, o tratamento do caminho de auth estava errado. ... As correções foram respeitar o caminho correto de auth e trocar o browse-all para o endpoint de busca, o que é uma forma muito mais sensata de evitar throttling não autenticado sem sentido.
...
[NOVA]: Sétimo: tratamento stale de provider no web_search. A ferramenta estava usando qualquer state de provider que tivesse sido embutido no startup em vez da config ativa de runtime.
...
[NOVA]: Oitavo: threading do Telegram. currentThreadTs agora é populado no fallback de tool-context de threading para tópicos de DM do Telegram, então ferramentas thread-aware de fato recebem o contexto correto do tópico.
...
[NOVA]: Vamos às mudanças menores, porque elas só são menores em área de superfície, não necessariamente em importância. Primeiro: Qwen e DashScope. O OpenClaw agora suporta endpoints padrão do DashScope para chaves de API do Qwen na China e globais, e o provider foi relabelado como Qwen (Alibaba Cloud Model Studio).
...
[NOVA]: Próximo: hardening de CSP. Hashes SHA-256 para blocos de script inline. Scripts inline bloqueados por padrão.

[ALLOY]: Se você roda OpenClaw atrás de um reverse proxy rígido, esta é a versão para a qual fazer upgrade.

[NOVA]: Isso importa para segurança de supply chain. Um script injetado não vai executar porque o hash não vai bater. ... Esse é o tipo de controle que não deixa uma demo mais bonita, mas deixa seu raio de dano menor.
...
[NOVA]: O tema Knot também recebeu atenção: conformidade de contraste WCAG 2.1 AA, ajuste da paleta preto e vermelho, ícones de config, degraus discretos de arredondamento.
...
[ALLOY]: openclaw doctor --fix. Primeiro, antes de qualquer outra coisa. E, honestamente, logo depois de cada etapa de upgrade também, se você estiver fazendo a sequência de forma limpa.

[NOVA]: Este é o comando âncora dessas releases. Não é algo bom de ter. É âncora.

[ALLOY]: Passo dois: dê grep em CLAWDBOT_* e MOLTBOT_* em todos os arquivos .env, arquivos Docker, units do systemd, shell profiles, toda superfície de startup que você tiver.

[NOVA]: Se os nomes antigos existirem, assuma que agora eles são config morta.

[ALLOY]: Passo três: verifique se existe ~/.moltbot. Se existir, mova o state para ~/.openclaw ou defina OPENCLAW_STATE_DIR explicitamente.

[NOVA]: Passo quatro: defina agents.defaults.imageGenerationModel.primary: "google/gemini-3-pro-image-preview". Não deixe geração de imagem num limbo indefinido.
...
[ALLOY]: Passo sete: se você tem plugins customizados, migre de openclaw/extension-api para openclaw/plugin-sdk/*. Leia os docs de migração. Não trate erros de compilação como mapa.
...
[ALLOY]: Passo nove: verifique jobs de cron usando --at --tz, especialmente se o DST já te queimou antes.
...
[ALLOY]: Doctor --fix primeiro. Todo o resto é condicional ao que você está rodando. Mas doctor --fix é incondicional.
...
[NOVA]: O que, para uma ferramenta como OpenClaw, importa mais do que novidade. ... Confiabilidade é uma feature. Consistência de nomenclatura é uma feature. Pressão de migração honesta é uma feature. Uma stack de navegador com menos pontes improvisadas é uma feature.

[ALLOY]: E, se você está no meio do upgrade agora, respire, faça o checklist, rode openclaw doctor --fix, e não pule as etapas de verificação só porque o serviço voltou a subir.

[NOVA]: Você pode encontrar as show notes, todos os links que mencionamos e o arquivo de episódios em tobyonfitnesstech.com. Isso é tobyonfitnesstech.com.
