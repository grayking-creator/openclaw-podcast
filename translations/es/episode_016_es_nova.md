...
[NOVA]: Bug uno: un usuario tenía MiniMax configurado como su modelo de razonamiento. La API upstream estaba devolviendo api_error. OpenClaw vio eso, decidió que debía ser transitorio, lo reintentó en silencio, nunca mostró la falla, nunca activó correctamente el fallback, y el usuario recibió un resultado degradado sin tener idea de que la llamada original había fallado.
...
[NOVA]: Exactamente. Y el bug dos era una clase distinta de dolor, pero igual de real. El usuario pega un token nuevo de OpenAI Codex, ve la confirmación, todo parece exitoso, reinicia el gateway, y el token se revierte a la credencial expirada.

[ALLOY]: Ese te hace sentir loco. Porque desde la perspectiva del usuario, hizo lo correcto. Pegó el token nuevo. La app dijo: sí, guardado. Luego, después del restart, no. Otra vez el token viejo y malo. ... Bajo el capó, el estado de auth en memoria del gateway estaba sobrescribiendo en el restart el valor recién guardado en disco.

[NOVA]: Ambos están corregidos en v2026.3.23. Pero para entender por qué ocurrieron, necesitas entender qué cambió v2026.3.22. Porque .22 es la grande. .22 es OpenClaw haciendo algo que probablemente debió haber hecho hace un año.

[ALLOY]: La purga del legado.

[NOVA]: La purga del legado. Los nombres viejos, las capas de compatibilidad, los caminos de transición raros, las muletas del browser relay, la masa amorfa del plugin SDK: una gran parte fue arrancada. ... Y creo que la manera correcta de enmarcar estas dos releases juntas es esta: .22 quita la piel muerta, y .23 se asegura de que la piel nueva no se agriete.
...
[NOVA]: Empecemos con el cambio más cargado emocionalmente, porque la gente se apega de forma rarísima a los nombres, incluso cuando esos nombres debieron haberse retirado para siempre hace mucho tiempo. Los nombres de entorno CLAWDBOT_* y MOLTBOT_* desaparecieron. No están deprecated. No se toleran con una advertencia. Desaparecieron.

[ALLOY]: Y quiero bajar un poco el ritmo ahí, porque si ejecutas OpenClaw en una sola laptop y en ningún otro lado, podrías escuchar eso y pensar, ok, renombro algunas variables, bien. Ese no es el modo real de falla. El modo real de falla es que tienes un archivo .env viejo en Docker Compose, o una unidad systemd toda oxidada en un VPS, o algo en un perfil de shell que no has mirado en ocho meses. Haces upgrade, OpenClaw arranca, y esos valores se ignoran en silencio.

[NOVA]: Sin error. Sin banner de migración.

[ALLOY]: Nada de “oye, vi CLAWDBOT_TOKEN y eso es obsoleto”. Solo config faltante. De pronto auth ya no cuadra, la ruta del state no está donde pensabas, tal vez un plugin no carga, tal vez un token parece ausente. ... Este es el tipo de ruptura que te pega a las dos de la mañana después de un upgrade en una máquina a la que no tocabas desde hace tiempo.

[NOVA]: Haz grep a tus archivos de env. Cada ambiente donde ejecutas OpenClaw. Cada host. Cada archivo de Compose. Cada unidad de arranque. Cada bootstrap de shell. Si tienes CLAWDBOT_* o MOLTBOT_*, trata eso como roto hasta corregirlo.

[ALLOY]: Misma historia con el viejo directorio de state ~/.moltbot. Esa ruta ya no es parte del futuro. Si tu state sigue viviendo ahí, OpenClaw no va a inferirlo mágicamente por ti después del upgrade. Muévelo a ~/.openclaw o define OPENCLAW_STATE_DIR explícitamente y termina con eso.
...
[NOVA]: Justo. ... El segundo gran breaking change en .22 es que ClawHub ahora es first-class, y el significado práctico de eso es mayor de lo que la redacción de marketing hace sonar. openclaw plugins install name ahora revisa ClawHub primero, y solo cae a npm si ClawHub no tiene el paquete.
...
[NOVA]: Tercero: la renovación total del Plugin SDK. Esto no es un mordisquito en los bordes. openclaw/extension-api desapareció. No hay compatibility shim. La nueva superficie es openclaw/plugin-sdk/*, con subpaths más estrechos y límites mucho más claros.
...
[ALLOY]: Y el descubrimiento de mensajes también cambió. describeMessageTool() ahora es obligatorio. El flujo viejo de listActions, getCapabilities, getToolSchema fue eliminado. ... Eso no es un rename. Eso es un cambio de contrato.

[NOVA]: El SDK nuevo de hecho es más limpio. De verdad. Los imports son más específicos. La intención es más clara. El modelo de runtime es más coherente. Pero sí tienes que migrar. Y si eres desarrollador de plugins, ve a leer docs.openclaw.ai/plugins/sdk-migration. No improvises de memoria.

[ALLOY]: Quiero subrayar eso. Esta no es la semana para hacer tu migración a pulso basándote en vibes.

[NOVA]: Cuarto bloque: endurecimiento de seguridad. Algunas de estas cosas son invisibles hasta que te salvan, lo que significa que no van a recibir tanto tiempo al aire, pero importan. El exec sandbox ahora bloquea MAVEN_OPTS, SBT_OPTS, GRADLE_OPTS, ANT_OPTS, además de GLIBC_TUNABLES y DOTNET_ADDITIONAL_DEPS. ... Eso es básicamente una barrida contra superficies de inyección de runtime que la gente olvida que existen.
...
[NOVA]: También hay un cambio sutil pero inteligente en la allowlist: time ahora es transparente en la evaluación de la allowlist. Así que time ./approved-script se vincula al script interno, no al wrapper time.

[ALLOY]: Ese es tan práctico. La gente envuelve comandos en time constantemente durante debugging. Antes, los wrappers podían crear edge cases raros de policy. Ahora evalúa la cosa que realmente querías ejecutar.

[NOVA]: Y el endurecimiento de voice webhook también se puso más estricto: rechazar firmas faltantes del provider antes de leer el body, con un límite pre-auth de 64KB y 5s. Lo cual es simplemente buena higiene de perímetro. No gastes recursos parseando basura no autenticada.
...
[NOVA]: El modelo de reemplazo es simplemente mejor. OpenClaw ahora se conecta directamente a una instancia de Chrome en ejecución o a un profile de usuario usando mecanismos estándar de CDP. No se necesita extensión. No hay relay personalizado en medio. ... Arquitectónicamente, esto es más limpio. Menos capas hechas a medida. Menos secretos escondidos en el tooling.

[ALLOY]: Pero, y esta es la advertencia práctica importante, si haces upgrade sin ejecutar primero openclaw doctor --fix, o al menos inmediatamente después, tu automatización de navegador puede romperse por completo. Y no siempre se va a romper de una forma obvia. No necesariamente vas a recibir un mensaje amigable que diga “extension relay removed”. Lo más probable es que simplemente falle al conectar, o los consent loops se pongan raros, o tu ruta de attach parezca medio viva y luego muera.

[NOVA]: openclaw doctor --fix lee tu config actual y migra los setups locales de browser en el host al modo moderno correcto: existing-session o user. Eso no es una recomendación cosmética. Es parte de la migración.

[ALLOY]: Y vale la pena aclarar los tres modos ahora. existing-session significa adjuntarse a un Chrome en ejecución. user significa lanzar con un profile de usuario. CDP puro para setups Docker, headless, sandbox o remotos: eso se mantiene básicamente igual.
...
[ALLOY]: Cierto. Y .23 de hecho prueba tu punto, porque una vez que .22 eliminó el camino viejo, .23 inmediatamente tuvo que hacer confiable el camino nuevo en el mundo real. ... Aquí importan dos correcciones. Primero, el timing del attach de pestañas. OpenClaw estaba tratando el handshake de Chrome MCP como si el navegador estuviera totalmente listo en el instante en que la conexión subía. En macOS, eso no siempre era cierto. Las pestañas existían, pero todavía no eran utilizables. Así que el primer attach podía hacer churn de consentimientos, disparar timeouts y en general sentirse embrujado.
...
[NOVA]: Además, si ejecutas automatización de navegador local en el host, deberías tratar doctor --fix como parte de la migración del navegador, no como una tarea general de mantenimiento. Está haciendo trabajo dirigido.
...
[NOVA]: Siguiente tema: generación de imagen. Esta es menos dramática que el tooling de navegador, pero te dice mucho sobre hacia dónde va OpenClaw. La skill empaquetada nano-banana-pro fue eliminada. Desapareció. Sin shim.

[ALLOY]: Lo que significa que si tenías workflows, prompts o docs internos que llaman nano-banana-pro, encuéntralos y reemplázalos. Esto es una ruptura dura. No asumas que hay un alias esperándote.

[NOVA]: La plataforma se está estandarizando sobre la herramienta core image_generate. Y filosóficamente, creo que esto es exactamente correcto. Una herramienta, backend configurable, superficie de invocación consistente. Mejor que cargar para siempre un wrapper de skill empaquetado solo porque la gente se acostumbró a su nombre.

[ALLOY]: Siempre y cuando configures la config key. Esta es la parte que la gente se salta. Necesitas agents.defaults.imageGenerationModel.primary: "google/gemini-3-pro-image-preview". Si no configuras eso, el comportamiento queda indefinido. Y “indefinido” en tierra de config nunca significa emocionante. Significa confuso.
...
[NOVA]: Crítica justa. ... También hay mejoras de marketplace envueltas en este cambio. Las instalaciones desde marketplace ahora son first-class, incluyendo la sintaxis plugin@marketplace y soporte para el registry de marketplace de Claude.
...
[NOVA]: Y los comandos de chat /plugins y /plugin restringidos al owner continúan exactamente ese mismo tema: darle a la plataforma una historia coherente única para descubrir, instalar y administrar extensiones.
...
[NOVA]: Para mí, la historia de la generación de imagen es esta: OpenClaw está pasando de una personalidad empaquetada a una capacidad configurada. ... Eso es madurez.
...
[NOVA]: También cambia cómo los equipos deberían pensar sobre portabilidad. Si tu workflow dice image_generate y tu backend se configura por separado, puedes migrar providers sin reescribir la lógica del workflow en sí.
...
[ALLOY]: Así que pegabas el token, veías verde, restart, expirado. Cada vez. ... Por eso este bug se sintió tan personal. Atacaba la confianza en el acto básico de guardar credenciales.
...
[NOVA]: Ejemplo concreto: programas un job a las 8 AM. Llega el DST. Antes de .23, esas “8 AM” podían convertirse en 7 AM o 9 AM dependiendo de cómo el scheduler interpretara el límite. ... Eso no es solo una discrepancia cosmética. Para una rutina diaria, es una promesa rota.

[ALLOY]: La corrección es que --at --tz ahora respeta la hora local de reloj a través de los cambios de DST. Y OpenClaw también rechaza --tz para --every, lo cual está bien porque la semántica de intervalos recurrentes y la semántica de hora local con timezone no son lo mismo.
...
[NOVA]: Cuarto: la corrección de 422 de Mistral. Configuraciones persistidas viejas de Mistral estaban cargando límites de output del tamaño del contexto que Mistral rechaza de plano. Resultado: errores 422 que se ven misteriosos si no conoces la historia de la config.

[ALLOY]: Y otra vez, openclaw doctor --fix está haciendo trabajo real aquí. Ahora detecta y repara esas configuraciones stale de Mistral.

[NOVA]: Otra razón para ejecutar doctor --fix. A veces la gente oye ese comando como una especie de doctor genérico, como si tal vez arreglara algunas cosas obvias. No. En esta tanda de releases, está codificando conocimiento de migración.

[ALLOY]: Quinto: ClawHub en macOS. Aquí hubo dos problemas. Las credenciales guardadas no estaban respetando correctamente la ruta de Application Support de macOS, y el comportamiento de browse-all estaba pegando contra rate limits 429 sin autenticar.

[NOVA]: Lo cual significa que la UI podía engañarte haciéndote pensar que ClawHub estaba vacío o roto de formas vagas.

[ALLOY]: La navegación de skills estaba cayendo silenciosamente a modo no autenticado en macOS. Veías listas vacías y asumías que no había skills, o asumías que tu instalación estaba rota, cuando en realidad el manejo de la ruta de auth estaba mal. ... Las correcciones fueron respetar la ruta correcta de auth y cambiar browse-all al endpoint de search, que es una forma mucho más sensata de evitar throttling no autenticado sin sentido.
...
[NOVA]: Séptimo: manejo stale del provider en web_search. La herramienta estaba usando el state del provider que hubiera quedado horneado en el arranque en vez de la config activa de runtime.
...
[NOVA]: Octavo: threading de Telegram. currentThreadTs ahora se llena en el fallback de tool-context de threading para temas de DM de Telegram, de modo que las herramientas con conciencia de hilos realmente reciben el contexto correcto del tema.
...
[NOVA]: Y creo que por eso los operadores deberían leer estas dos releases como una sola historia con dos capítulos. El capítulo uno dice: “quitamos los viejos compromisos”. El capítulo dos dice: “arreglamos los lugares donde los supuestos nuevos todavía tenían asperezas”. ... Ese es un ritmo de desarrollo mucho más honesto que fingir que la gran limpieza aterrizó perfecta desde el día uno.
...
[NOVA]: Vamos con los cambios más pequeños, porque son pequeños solo en superficie, no necesariamente en importancia. Primero: Qwen y DashScope. OpenClaw ahora soporta endpoints estándar de DashScope para China y claves de API globales de Qwen, y el provider fue renombrado como Qwen (Alibaba Cloud Model Studio).
...
[NOVA]: Siguiente: endurecimiento de CSP. Hashes SHA-256 para bloques de script inline. Scripts inline bloqueados por default.

[ALLOY]: Si ejecutas OpenClaw detrás de un reverse proxy estricto, esta es la versión a la que deberías hacer upgrade.

[NOVA]: Esto importa para la seguridad de la supply chain. Un script inyectado no va a ejecutarse porque el hash no va a coincidir. ... Este es el tipo de controles que no hacen que una demo se vea más bonita, pero sí hacen más pequeño tu radio de impacto.
...
[NOVA]: El tema Knot también recibió atención: cumplimiento de contraste WCAG 2.1 AA, ajuste de la paleta negro y rojo, íconos de config, niveles discretos de redondez.
...
[ALLOY]: openclaw doctor --fix. Primero, antes de cualquier otra cosa. Y honestamente, inmediatamente después de cada etapa del upgrade si estás haciendo la secuencia de forma limpia.

[NOVA]: Es el comando ancla de estas releases. No es un nice-to-have. Es el ancla.

[ALLOY]: Paso dos: haz grep de CLAWDBOT_* y MOLTBOT_* en todos los archivos .env, archivos de Docker, unidades systemd, perfiles de shell, cada superficie de arranque que tengas.

[NOVA]: Si los nombres viejos existen, asume que ahora son config muerta.

[ALLOY]: Paso tres: revisa si existe ~/.moltbot. Si existe, mueve el state a ~/.openclaw o define OPENCLAW_STATE_DIR explícitamente.

[NOVA]: Paso cuatro: define agents.defaults.imageGenerationModel.primary: "google/gemini-3-pro-image-preview". No dejes la generación de imagen en un limbo indefinido.
...
[ALLOY]: Paso siete: si tienes plugins personalizados, migra de openclaw/extension-api a openclaw/plugin-sdk/*. Lee los docs de migración. No trates los errores de compilación como si fueran un mapa.
...
[ALLOY]: Paso nueve: verifica los jobs de cron que usan --at --tz, especialmente si el DST ya te quemó antes.
...
[ALLOY]: Primero Doctor --fix. Todo lo demás es condicional a lo que estés ejecutando. Pero doctor --fix es incondicional.
...
[NOVA]: Lo cual, para una herramienta como OpenClaw, importa más que la novedad. ... La confiabilidad es una feature. La consistencia de nombres es una feature. La presión honesta de migración es una feature. Una stack de navegador con menos puentes improvisados es una feature.

[ALLOY]: Y si estás justo en medio del upgrade ahora mismo, respira, haz el checklist, ejecuta openclaw doctor --fix, y no te saltes los pasos de verificación solo porque el servicio volvió a levantarse.

[NOVA]: Puedes encontrar las notas del episodio, todos los links que mencionamos y el archivo de episodios en tobyonfitnesstech.com. Eso es tobyonfitnesstech.com.
