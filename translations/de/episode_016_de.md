...
NOVA: Bug eins: Ein User hatte [EMPHASIS]MiniMax[/EMPHASIS] als sein Reasoning-Modell konfiguriert. Die Upstream-API warf [EMPHASIS]api_error[/EMPHASIS]. OpenClaw sah das, entschied, es müsse transient sein, versuchte es still noch einmal, machte den Fehler nie sichtbar, löste Fallback nie korrekt aus, und der User bekam ein degradiertes Ergebnis zurück, ohne jede Ahnung, dass der ursprüngliche Aufruf fehlgeschlagen war.
...
NOVA: Genau. Und Bug zwei war eine andere Art von Schmerz, aber genauso real. Ein User fügt ein frisches [EMPHASIS]OpenAI Codex[/EMPHASIS]-Token ein, sieht die Bestätigung, alles wirkt erfolgreich, startet das Gateway neu, und das Token ist wieder auf die abgelaufene Credential zurückgeschnappt.

ALLOY: Bei dem hier fühlst du dich verrückt. Denn aus Sicht des Users hat er alles richtig gemacht. Er hat das neue Token eingefügt. Die App sagte: ja, gespeichert. Dann nach dem Neustart: nein. Wieder das alte kaputte Token. [PAUSE] Unter der Haube hat der In-Memory-Auth-State des Gateways beim Neustart den frisch auf Platte gespeicherten Wert überschrieben.

NOVA: Beides ist in [EMPHASIS]v2026.3.23[/EMPHASIS] behoben. Aber um zu verstehen, warum das passiert ist, musst du verstehen, was [EMPHASIS]v2026.3.22[/EMPHASIS] geändert hat. Denn .22 ist die große. .22 ist OpenClaw, das etwas tut, was es wahrscheinlich schon vor einem Jahr hätte tun sollen.

ALLOY: Die Legacy-Säuberung.

NOVA: Die Legacy-Säuberung. Die alten Namen, die Kompatibilitätsschichten, die seltsamen Übergangspfade, die Browser-Relay-Krücken, der Plugin-SDK-Brei — vieles davon wurde rausgerissen. [PAUSE] Und ich glaube, die richtige Art, diese beiden Releases zusammen zu rahmen, ist diese: .22 entfernt die tote Haut, und .23 sorgt dafür, dass die neue Haut nicht aufreißt.
...
## [02:30-11:00] Die Legacy-Säuberung — v2026.3.22 Breaking Changes

NOVA: Fangen wir mit der emotional aufgeladensten Änderung an, denn Menschen hängen seltsam stark an Namen, selbst wenn diese Namen längst für immer hätten ausgemustert werden sollen. Die Environment-Namen [EMPHASIS]CLAWDBOT_*[/EMPHASIS] und [EMPHASIS]MOLTBOT_*[/EMPHASIS] sind weg. Nicht deprecated. Nicht mit einer Warnung toleriert. Weg.

ALLOY: Und ich will das bewusst langsamer machen, denn wenn du OpenClaw auf einem Laptop und sonst nirgendwo betreibst, hörst du das vielleicht und denkst: okay, ein paar Variablen umbenennen, passt. Das ist nicht der echte Failure Mode. Der echte Failure Mode ist, dass du eine alte [EMPHASIS].env[/EMPHASIS]-Datei in Docker Compose hast oder eine verkrustete [EMPHASIS]systemd[/EMPHASIS]-Unit auf einem VPS oder irgendetwas in einem Shell-Profil, das du seit acht Monaten nicht mehr angesehen hast. Du machst ein Upgrade, OpenClaw startet, und diese Werte werden stillschweigend ignoriert.

NOVA: Kein Fehler. Kein Migrationsbanner.

ALLOY: Kein „hey, ich habe [EMPHASIS]CLAWDBOT_TOKEN[/EMPHASIS] gesehen und das ist veraltet“. Einfach fehlende Config. Plötzlich passt auth nicht mehr, der State-Pfad ist nicht da, wo du ihn vermutet hast, vielleicht lädt ein Plugin nicht, vielleicht wirkt ein Token abwesend. [PAUSE] Das ist die Art von Bruch, die dich um zwei Uhr morgens nach einem Upgrade auf einer Maschine erwischt, die du lange nicht angefasst hast.

NOVA: Mach grep über deine Env-Dateien. In jeder Umgebung, in der du OpenClaw betreibst. Auf jedem Host. In jeder Compose-Datei. In jeder Startup-Unit. In jedem Shell-Bootstrap. Wenn du [EMPHASIS]CLAWDBOT_*[/EMPHASIS] oder [EMPHASIS]MOLTBOT_*[/EMPHASIS] hast, behandle das als kaputt, bis es korrigiert ist.

ALLOY: Dasselbe gilt für das alte [EMPHASIS]~/.moltbot[/EMPHASIS]-State-Verzeichnis. Dieser Pfad gehört nicht mehr zur Zukunft. Wenn dein State noch dort liegt, wird OpenClaw das nach dem Upgrade nicht magisch für dich erschließen. Verschiebe ihn nach [EMPHASIS]~/.openclaw[/EMPHASIS] oder setze [EMPHASIS]OPENCLAW_STATE_DIR[/EMPHASIS] explizit und mach damit Schluss.
...
NOVA: Fair. [PAUSE] Die zweite große Breaking Change in .22 ist, dass [EMPHASIS]ClawHub[/EMPHASIS] jetzt first-class ist, und die praktische Bedeutung davon ist größer, als das Marketing-Wording vermuten lässt. [EMPHASIS]openclaw plugins install name[/EMPHASIS] prüft jetzt zuerst ClawHub und fällt nur dann auf npm zurück, wenn ClawHub das Paket nicht hat.
...
NOVA: Es gibt jetzt auch native Befehle: [EMPHASIS]openclaw skills search[/EMPHASIS], [EMPHASIS]openclaw skills install[/EMPHASIS], [EMPHASIS]openclaw skills update[/EMPHASIS]. [PAUSE] Und für mich ist das OpenClaw, das das Produkt endlich mit seinem beabsichtigten Ökosystem in Einklang bringt. ClawHub sollte immer das Zuhause für Skills sein. Dieses Release macht das real statt bloß aspirativ.
...
NOVA: Drittens: der [EMPHASIS]Plugin SDK[/EMPHASIS]-Umbau. Das ist kein kleines Knabbern an den Rändern. [EMPHASIS]openclaw/extension-api[/EMPHASIS] ist weg. Es gibt keinen Compatibility-Shim. Die neue Oberfläche ist [EMPHASIS]openclaw/plugin-sdk/*[/EMPHASIS] mit engeren Subpaths und viel klareren Grenzen.
...
ALLOY: Und auch die Message-Discovery hat sich geändert. [EMPHASIS]describeMessageTool()[/EMPHASIS] ist jetzt erforderlich. Der alte Ablauf mit [EMPHASIS]listActions[/EMPHASIS], [EMPHASIS]getCapabilities[/EMPHASIS], [EMPHASIS]getToolSchema[/EMPHASIS] ist entfernt. [PAUSE] Das ist keine Umbenennung. Das ist eine Vertragsänderung.

NOVA: Das neue SDK ist tatsächlich sauberer. Wirklich. Die Imports sind enger. Die Intention ist klarer. Das Runtime-Modell ist kohärenter. Aber du musst migrieren. Und wenn du Plugin-Developer bist, lies [EMPHASIS]docs.openclaw.ai/plugins/sdk-migration[/EMPHASIS]. Improvisiere nicht aus dem Gedächtnis.

ALLOY: Ich will das unterstreichen. Das ist nicht die Woche, um deine Migration frei Hand nach Gefühl hinzuschreiben.

NOVA: Vierter Block: Security-Hardening. Manches davon ist unsichtbar, bis es dich rettet, was bedeutet, dass es nicht so viel Airtime bekommt, aber es ist wichtig. Der Exec-Sandbox blockiert jetzt [EMPHASIS]MAVEN_OPTS[/EMPHASIS], [EMPHASIS]SBT_OPTS[/EMPHASIS], [EMPHASIS]GRADLE_OPTS[/EMPHASIS], [EMPHASIS]ANT_OPTS[/EMPHASIS] sowie [EMPHASIS]GLIBC_TUNABLES[/EMPHASIS] und [EMPHASIS]DOTNET_ADDITIONAL_DEPS[/EMPHASIS]. [PAUSE] Das ist im Grunde eine Bereinigung gegen Runtime-Injection-Oberflächen, von denen die Leute vergessen, dass sie existieren.
...
NOVA: Es gibt auch eine subtile, aber clevere Allowlist-Änderung: [EMPHASIS]time[/EMPHASIS] ist jetzt transparent in der Allowlist-Auswertung. Also bindet [EMPHASIS]time ./approved-script[/EMPHASIS] an das innere Skript, nicht an den [EMPHASIS]time[/EMPHASIS]-Wrapper.

ALLOY: Das ist so praktisch. Leute wrappen beim Debugging ständig Befehle mit [EMPHASIS]time[/EMPHASIS]. Früher konnten Wrapper seltsame Policy-Edge-Cases erzeugen. Jetzt bewertet es das Ding, das du tatsächlich ausführen wolltest.

NOVA: Und auch das Voice-Webhook-Hardening wurde strenger: fehlende Provider-Signaturen werden vor dem Lesen des Bodys abgelehnt, mit einem [EMPHASIS]64KB[/EMPHASIS]- und [EMPHASIS]5s[/EMPHASIS]-Pre-Auth-Limit. Das ist einfach saubere Perimeter-Hygiene. Verbrate keine Ressourcen darauf, nicht authentifizierten Müll zu parsen.
...
## [11:00-17:00] Chrome MCP: Die Extension ist tot

NOVA: Wir müssen echte Zeit auf Browser-Tooling verwenden, denn für viele User ist das die größte einzelne operative Änderung in diesem Doppelt-Release. Das Legacy-Chrome-Extension-Relay ist weg. [EMPHASIS]driver: "extension"[/EMPHASIS], gebündelte Extension-Assets, [EMPHASIS]browser.relayBindHost[/EMPHASIS] — alles entfernt.

ALLOY: Und falls du damals nicht dabei warst: Hier ist, was die Extension tatsächlich war. OpenClaw hat früher eine Chrome-Extension ausgeliefert, die wie ein Relay für [EMPHASIS]CDP[/EMPHASIS], das Chrome DevTools Protocol, fungierte. Du hast die Extension manuell installiert, Browser-Berechtigungen vergeben, und sie diente als Brücke zwischen OpenClaw und dem Browser.
...
NOVA: Das Ersatzmodell ist einfach besser. OpenClaw verbindet sich jetzt direkt mit einer laufenden Chrome-Instanz oder mit einem User-Profil über Standard-CDP-Mechanismen. Keine Extension nötig. Kein Custom-Relay in der Mitte. [PAUSE] Architektonisch ist das sauberer. Weniger maßgeschneiderte Schichten. Weniger im Tooling versteckte Geheimnisse.

ALLOY: Aber — und das ist die wichtige praktische Warnung — wenn du ein Upgrade machst, ohne vorher [EMPHASIS]openclaw doctor --fix[/EMPHASIS] auszuführen, oder zumindest direkt danach, kann deine Browser-Automatisierung komplett kaputtgehen. Und sie geht nicht immer auf offensichtliche Weise kaputt. Du bekommst nicht zwangsläufig eine freundliche Nachricht wie „extension relay removed“. Wahrscheinlicher ist, dass es einfach nicht verbindet oder Consent-Schleifen sich merkwürdig verhalten oder dein Attach-Pfad halb lebendig aussieht und dann stirbt.

NOVA: [EMPHASIS]openclaw doctor --fix[/EMPHASIS] liest deine aktuelle Config und migriert host-lokale Browser-Setups in den richtigen modernen Modus: [EMPHASIS]existing-session[/EMPHASIS] oder [EMPHASIS]user[/EMPHASIS]. Das ist keine kosmetische Empfehlung. Es ist Teil der Migration.

ALLOY: Und es lohnt sich, die drei Modi jetzt klarzustellen. [EMPHASIS]existing-session[/EMPHASIS] bedeutet, an ein laufendes Chrome anzudocken. [EMPHASIS]user[/EMPHASIS] bedeutet, mit einem User-Profil zu starten. Rohes [EMPHASIS]CDP[/EMPHASIS] für Docker-, headless-, Sandbox- oder Remote-Setups — das bleibt im Wesentlichen unverändert.
...
NOVA: Außerdem solltest du, wenn du host-lokale Browser-Automatisierung betreibst, [EMPHASIS]doctor --fix[/EMPHASIS] als Teil der Browser-Migration behandeln, nicht als allgemeine Wartungsaufgabe. Es macht gezielte Arbeit.

ALLOY: Ja. Kein optionales Housekeeping. Ein Migrationsschritt.

NOVA: Und es gibt auch eine größere Lektion in diesem Browser-Wechsel. Browser-Automatisierung ist eines dieser Gebiete, in denen Leute absurde Komplexität tolerieren, weil der Payoff so hoch ist. Sie installieren eine Extension, pinnen eine Version, segnen ein Profil, schleppen seltsame Launch-Flags mit sich herum — alles, solange der Browser gehorcht. [PAUSE] Aber jeder versteckte Workaround wird zu technischer Schuld mit Benutzeroberfläche.
...
## [17:00-22:00] Image Gen wird standardisiert

NOVA: Als Nächstes: Bildgenerierung. Das hier ist weniger dramatisch als Browser-Tooling, aber es sagt dir viel darüber, wohin OpenClaw geht. Die gebündelte Skill [EMPHASIS]nano-banana-pro[/EMPHASIS] wurde entfernt. Weg. Kein Shim.

ALLOY: Das bedeutet: Wenn du Workflows, Prompts oder interne Docs hattest, die [EMPHASIS]nano-banana-pro[/EMPHASIS] aufrufen, finde sie und ersetze sie. Das ist ein harter Bruch. Geh nicht davon aus, dass ein Alias auf dich wartet.

NOVA: Die Plattform standardisiert sich auf das Core-Tool [EMPHASIS]image_generate[/EMPHASIS]. Und philosophisch halte ich das für genau richtig. Ein Tool, konfigurierbares Backend, konsistente Invocation-Oberfläche. Besser, als für immer einen gebündelten Skill-Wrapper mitzuschleppen, nur weil Menschen sich an seinen Namen gewöhnt haben.

ALLOY: Solange du den Config-Key setzt. Das ist der Teil, den Menschen überspringen. Du brauchst [EMPHASIS]agents.defaults.imageGenerationModel.primary: "google/gemini-3-pro-image-preview"[/EMPHASIS]. Wenn du das nicht setzt, ist das Verhalten undefiniert. Und undefiniert im Config-Land bedeutet nie spannend. Es bedeutet verwirrend.
...
NOVA: Faire Kritik. [PAUSE] Es gibt in diesem Shift auch Marketplace-Verbesserungen. Marketplace-Installationen sind jetzt first-class, einschließlich [EMPHASIS]plugin@marketplace[/EMPHASIS]-Syntax und Claude-Marketplace-Registry-Support.
...
NOVA: Und Owner-gated [EMPHASIS]/plugins[/EMPHASIS]- und [EMPHASIS]/plugin[/EMPHASIS]-Chat-Befehle setzen genau dieses Thema fort: der Plattform eine einzige kohärente Geschichte für das Entdecken, Installieren und Verwalten von Extensions zu geben.
...
NOVA: Für mich ist die Story bei der Bildgenerierung diese: OpenClaw bewegt sich von gebündelter Persönlichkeit hin zu konfigurierter Fähigkeit. [PAUSE] Das ist Reife.
...
NOVA: Es verändert auch, wie Teams über Portabilität nachdenken sollten. Wenn dein Workflow [EMPHASIS]image_generate[/EMPHASIS] sagt und dein Backend separat konfiguriert ist, kannst du Provider migrieren, ohne die Workflow-Logik selbst umzuschreiben.
...
## [22:00-31:00] Dafür sorgen, dass es hält — Der .23 Reliability Pass
...
NOVA: Der Fix ist präzise. Es ist nicht: „Hört auf, MiniMax neu zu versuchen.“ Es ist: „Versuche nur dann erneut, wenn der Fehler tatsächlich transient aussieht.“ [PAUSE] Das ist die Art von Fix, der ich vertraue, weil er das ursprüngliche Designziel — Resilienz — bewahrt und gleichzeitig die schlampige Klassifizierung entfernt, die Resilienz wie Verschleierung wirken ließ.
...
ALLOY: Also fügst du das Token ein, siehst grün, startest neu, abgelaufen. Jedes Mal. [PAUSE] Deshalb fühlte sich dieser Bug so persönlich an. Er griff das Vertrauen in den grundlegenden Akt des Speicherns von Credentials an.
...
NOVA: Konkretes Beispiel: Du planst einen Job für [EMPHASIS]8 AM[/EMPHASIS]. DST schlägt zu. Vor .23 konnten diese „8 AM“ zu [EMPHASIS]7 AM[/EMPHASIS] oder [EMPHASIS]9 AM[/EMPHASIS] werden, abhängig davon, wie der Scheduler die Grenze interpretierte. [PAUSE] Das ist nicht nur eine kosmetische Abweichung. Für eine tägliche Routine ist das ein gebrochenes Versprechen.

ALLOY: Der Fix ist, dass [EMPHASIS]--at --tz[/EMPHASIS] jetzt lokale Wanduhrzeit über DST-Grenzen hinweg respektiert. Und OpenClaw lehnt [EMPHASIS]--tz[/EMPHASIS] auch für [EMPHASIS]--every[/EMPHASIS] ab, was gut ist, weil die Semantik wiederkehrender Intervalle und die Semantik lokaler Zeitzonen-Wanduhrzeit nicht dasselbe sind.
...
NOVA: Viertens: der Mistral-[EMPHASIS]422[/EMPHASIS]-Fix. Ältere persistierte Mistral-Configs trugen Output-Limits in Kontextgröße mit sich herum, die Mistral sofort ablehnt. Ergebnis: [EMPHASIS]422[/EMPHASIS]-Fehler, die mysteriös wirken, wenn du die Config-Herkunft nicht kennst.

ALLOY: Und wieder macht [EMPHASIS]openclaw doctor --fix[/EMPHASIS] hier echte Arbeit. Es erkennt und repariert jetzt diese veralteten Mistral-Configs.

NOVA: Noch ein Grund, [EMPHASIS]doctor --fix[/EMPHASIS] laufen zu lassen. Manche hören diesen Befehl als generelles Doktern, als würde er vielleicht ein paar offensichtliche Dinge aufräumen. Nein. In dieser Release-Reihe kodifiziert er Migrationswissen.

ALLOY: Fünftens: ClawHub auf macOS. Hier gibt es zwei Probleme. Gespeicherte Credentials respektierten den macOS-Application-Support-Pfad nicht korrekt, und das Browse-all-Verhalten lief gegen unauthentifizierte [EMPHASIS]429[/EMPHASIS]-Rate-Limits.
...
NOVA: Siebtens: veraltete Provider-Behandlung in [EMPHASIS]web_search[/EMPHASIS]. Das Tool verwendete den Provider-State, der beim Startup eingebacken worden war, statt der aktiven Runtime-Config.
...
NOVA: Achtens: Telegram-Threading. [EMPHASIS]currentThreadTs[/EMPHASIS] wird jetzt im Threading-Tool-Context-Fallback für Telegram-DM-Themen befüllt, sodass thread-aware Tools tatsächlich den richtigen Themenkontext erhalten.
...
## [31:00-35:00] Qwen, CSP und die kleinen Dinge

NOVA: Schauen wir auf die kleineren Änderungen, denn kleiner sind sie nur in der Oberfläche, nicht unbedingt in ihrer Wichtigkeit. Erstens: [EMPHASIS]Qwen[/EMPHASIS] und [EMPHASIS]DashScope[/EMPHASIS]. OpenClaw unterstützt jetzt Standard-DashScope-Endpoints für China und globale Qwen-API-Keys, und der Provider wurde in [EMPHASIS]Qwen (Alibaba Cloud Model Studio)[/EMPHASIS] umbenannt.
...
NOVA: Als Nächstes: [EMPHASIS]CSP[/EMPHASIS]-Hardening. SHA-256-Hashes für Inline-Script-Blöcke. Inline-Scripts standardmäßig blockiert.

ALLOY: Wenn du OpenClaw hinter einem strikten Reverse Proxy betreibst, ist das die Version, auf die du upgraden solltest.

NOVA: Das ist wichtig für Supply-Chain-Sicherheit. Ein injiziertes Script wird nicht ausgeführt, weil der Hash nicht passt. [PAUSE] Das sind die Arten von Kontrollen, die eine Demo nicht hübscher machen, aber deinen Blast Radius kleiner.
...
NOVA: Auch das [EMPHASIS]Knot[/EMPHASIS]-Theme bekam Aufmerksamkeit: [EMPHASIS]WCAG 2.1 AA[/EMPHASIS]-Kontrast-Compliance, Feintuning der Schwarz-Rot-Palette, Config-Icons, diskrete Rundungsstufen.
...
## [35:00-38:00] Die Upgrade-Checkliste
...
ALLOY: [EMPHASIS]openclaw doctor --fix[/EMPHASIS]. Zuerst, vor allem anderen. Und ehrlich gesagt direkt nach jeder Upgrade-Stufe, wenn du die Sequenz sauber durchziehst.

NOVA: Das ist der Ankerbefehl für diese Releases. Kein Nice-to-have. Anker.

ALLOY: Schritt zwei: Suche per grep nach [EMPHASIS]CLAWDBOT_*[/EMPHASIS] und [EMPHASIS]MOLTBOT_*[/EMPHASIS] in allen [EMPHASIS].env[/EMPHASIS]-Dateien, Docker-Dateien, systemd-Units, Shell-Profilen, jeder Startup-Oberfläche, die du hast.

NOVA: Wenn die alten Namen existieren, geh davon aus, dass sie jetzt tote Config sind.

ALLOY: Schritt drei: Prüfe auf [EMPHASIS]~/.moltbot[/EMPHASIS]. Wenn es existiert, verschiebe den State nach [EMPHASIS]~/.openclaw[/EMPHASIS] oder setze [EMPHASIS]OPENCLAW_STATE_DIR[/EMPHASIS] explizit.

NOVA: Schritt vier: setze [EMPHASIS]agents.defaults.imageGenerationModel.primary: "google/gemini-3-pro-image-preview"[/EMPHASIS]. Lass die Bildgenerierung nicht in undefiniertem Limbo hängen.
...
ALLOY: Schritt sieben: Wenn du eigene Plugins hast, migriere von [EMPHASIS]openclaw/extension-api[/EMPHASIS] zu [EMPHASIS]openclaw/plugin-sdk/*[/EMPHASIS]. Lies die Migrationsdocs. Behandle Compiler-Fehler nicht wie eine Karte.
...
ALLOY: Schritt neun: Verifiziere Cron-Jobs, die [EMPHASIS]--at --tz[/EMPHASIS] verwenden, besonders wenn DST dich schon einmal erwischt hat.
...
ALLOY: Zuerst Doctor [EMPHASIS]--fix[/EMPHASIS]. Alles andere hängt davon ab, was du betreibst. Aber [EMPHASIS]doctor --fix[/EMPHASIS] ist bedingungslos.
...
NOVA: Was bei einem Tool wie OpenClaw wichtiger ist als Neuheit. [PAUSE] Zuverlässigkeit ist ein Feature. Konsistente Benennung ist ein Feature. Ehrlicher Migrationsdruck ist ein Feature. Ein Browser-Stack mit weniger improvisierten Brücken ist ein Feature.

ALLOY: Und wenn du gerade mitten im Upgrade steckst, hol tief Luft, mach die Checkliste, führe [EMPHASIS]openclaw doctor --fix[/EMPHASIS] aus und überspringe die Verifikationsschritte nicht bloß, weil der Service wieder hochgekommen ist.

NOVA: Du findest die Show Notes, alle erwähnten Links und das Episodenarchiv unter [EMPHASIS]tobyonfitnesstech.com[/EMPHASIS]. Das ist [EMPHASIS]tobyonfitnesstech.com[/EMPHASIS].
